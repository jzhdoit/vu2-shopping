    ...
    <div class="order-list-item" v-if="item.order_id">
      ...
      <div class="actions">
        <div v-if="item.order_status === 10">
            ...
            <span v-else-if="item.delivery_status === 20 || item.delivery_status === 30" @click="receive">确认收货</span>
        </div>
        <div v-if="item.order_status === 30">
            <span @click="$router.push(`/orderComment?orderId=${item.order_id}`)">评价</span>
        </div>
      </div>
    </div>
  </template>
...
<script>
import { cancelOrder, confirmReceive } from '@/api/order'
...
export default {
  ...
  methods: {
    ...
    async receive () {
      const res = await confirmReceive(this.item.order_id)
      console.log(res)
    }
  }
}
