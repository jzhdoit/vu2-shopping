  ...
  <div class="prodetail">
    ...
    <van-swipe :autoplay="3000" @change="onChange">
      <van-swipe-item v-for="(image,index) in images" :key="index">
        <img v-lazy="image.external_url"  @click="showImagePreview(index)"
         alt="">
      </van-swipe-item>
      <template #indicator>
    ...
    <div class="info">
      ...
      <div class="service">
        <div class="left-words">
          <span><van-icon name="passed">七天无理由退货</van-icon></span>
    ...
    <van-action-sheet v-model="showPannel" :title="mode==='cart' ? '加入购物车' : '立即购买'">
      <div class="product">
        ...
        <div class="btn-none" v-else>该商品已经抢完</div>
      </div>
    </van-action-sheet>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    imagePreview () {
      return this.images.map(item => item.external_url)
    }
  },
  methods: {
    // 显示预览图
    ...
    async getComments () {
      const { data: { list, total } } = await getProComments(this.goodsId)
    ...
    async addCart () {
      ...
      this.cartTotal = data.cartTotal
      this.$store.commit('cart/cartTotal', data.cartTotal)
      Toast('加入购物车成功')
